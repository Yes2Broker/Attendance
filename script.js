...
function updateCards(data) {
    ...
    data.forEach(r => {
        ...
        // Count only those short day rows that are also marked as late.
        // This avoids subtracting unrelated short-day entries from the late count.
        if (r.status === "Present" && r.half === "Short Day" && r.late === "Yes") {
            overlappedLateShorts++;
        }
    ...
    let effectiveLate = late - overlappedLateShorts; // remove only overlapping late-short days
    if (effectiveLate < 0) {
        effectiveLate = 0;
    }
